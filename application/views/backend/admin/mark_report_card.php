		...
		<div class="panel panel-info">
            <div class="panel-heading"> <i class="fa fa-plus"></i>&nbsp;&nbsp;<?php echo get_phrase('Enter Student Score SHEET');?></div>
                <div class="panel-body table-responsive">
                	...
                	<?php echo form_open(base_url() . 'admin/mark_report_card' , array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_top', 'enctype' => 'multipart/form-data'));?>
...
<div class="col-sm-12">
        ...
        <div class="panel-heading"> <i class="fa fa-plus"></i>&nbsp;&nbsp;<?php echo get_phrase('enter_student_score'); ?></div>
            <div class="panel-body table-responsive">
                    ...
                    <table cellpadding="0" cellspacing="0" border="0" class="table">
                            <thead>
                                <tr>
                                <td><?php echo get_phrase('total_scores');?></td>
                                <td><?php echo get_phrase('lowest_in_class');?></td>
                                <td><?php echo get_phrase('highest_in_class');?></td>
                                <td><?php echo get_phrase('class_averages');?></td>
                                <td><?php echo get_phrase('total_scores');?></td>
                                <td><?php echo get_phrase('cum_average_score');?></td>
                                <td><?php echo get_phrase('3rd_term_average');?></td>
    ...
    <?php $select_subject_with_class_id  =   $this->crud_model->get_subjects_by_class($class_id);
       ...
       ?>
        ...
        <?php echo form_open(base_url() . 'admin/mark_report_card/'. $exam_id . '/' . $class_id);?>
                    <tr>
                    <td><?php echo get_phrase('total_scores');?></td>
                                        
                                          <td>
                                            <textarea name="lowest_in_class_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['lowest_in_class'];?></textarea>
                                    </td>
                                    <td>
                                            <textarea name="highest_in_class_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['highest_in_class'];?></textarea>
                                    </td>
                                    <td>
                                            <textarea name="class_averages_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['class_averages'];?></textarea>
                                    </td>
                                    <td>
                                            <textarea name="total_scores_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['total_scores'];?></textarea>
                                    </td>
                                    <td>
                                            <textarea name="cum_average_score_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['cum_average_score'];?></textarea>
                                    </td>
                                    <td>
                                            <textarea name="3rd_term_average_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['3rd_term_average'];?></textarea>
                                    </td>
        
                                        <td>
                                            <textarea name="attendance_<?php echo $class_subject_exam_student['subject_id'];?>" class="form-control"><?php echo $general_select['attendance'];?></textarea>
                                    </td>
...
function class_average_change() {
  var class_average = document.getElementsByClassName('class_average');
    for (var i = class_average.length - 1; i >= 0; i--) {
      var value = class_average[i].value;
        if (value > 100) {
            class_average[i].value = 0;
                $('#error_message').show();
        }
    }
}
